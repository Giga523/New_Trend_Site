...
class ElementSdkImpl {
    async connectToParent() {
        const errorService = new _error_service_ts__WEBPACK_IMPORTED_MODULE_4__.ErrorServiceImpl(this.logger);
        const result = await (0,_apps_developing_sandbox_comms_connect_to_parent_ts__WEBPACK_IMPORTED_MODULE_1__.connectToParent)(errorService, TARGET_ORIGIN, window, _element_comms_client_ts__WEBPACK_IMPORTED_MODULE_2__.ELEMENT_CLIENT_ID);
        if (!result.ok) throw new Error('Failed to connect to parent');
        ...
        if (ResizeObserver == null) this.logger.warn('ResizeObserver is not supported');
        else {
        ...
        const elementClientHandler = {
            ...
            handleSetCapability: async (request)=>{
                if (!this.element) throw new Error('Element is uninitialised');
        ...
        this.commsClient = new _element_comms_client_ts__WEBPACK_IMPORTED_MODULE_2__.ElementClientComms(result.value, elementClientHandler, this.logger);
    }
    /** @inheritDoc */ get config() {
        if (!this.element) throw new Error('Element is uninitialised');
        return this.element.config;
    }
    /** @inheritDoc */ init(element) {
        this.logger.info('Initializing Element');
        this.element = new Element(element.defaultConfig, element.render, element.mapToCapabilities, element.mapToEditPanelValues);
    ...
    /** @inheritDoc */ async setConfig(config) {
        ...
        if (!this.element) return _sdk_result__WEBPACK_IMPORTED_MODULE_6__.SdkResult.error(new Error('Element is uninitialised'));
    ...
    constructor(){
        this.logger = new _logger_ts__WEBPACK_IMPORTED_MODULE_5__.Logger('Element');
...
class Element {
    ...
    setConfig(config) {
        ...
        return this.render(this._config);
    }
    ...
    constructor(defaultConfig, render, mapToCapabilities, mapToEditPanelValues){
...
window.elementSdk = new ElementSdkImpl();
